...
function displayMovieVideos(movieId) {
  ...
  console.log(movieVideos); // the whole object that I created manually with API and Youtube IDs
  console.log(videoIds); // the Youtube ID of the selected movie
  if (videoIds.length > 0) {
    // condition checks that array is not empty
    videoIds.forEach((videoId) => {
      // is it fine to use a for each loop even if the array contains only one element?
    ...
    noVideoMessage.textContent = "This video is not available.";
